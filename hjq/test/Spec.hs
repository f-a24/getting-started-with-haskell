...
jqFilterParserTest =
  TestList
    [ "jqFilterParser test 1" ~: parseJqFilter "." ~?= Right JqNil, -- .が来たらJqNil
      "jqFilterParser test 2" ~: parseJqFilter ".[0]" ~?= Right (JqIndex 0 JqNil), -- .[0]が来たらJqIndex 0 JqNil
...
jqQueryParserTest =
  TestList
    [ "jqQueryParser test 1" ~: parseJqQuery "[]" ~?= Right (JqQueryArray []), -- []で空配列
      "jqQueryParser test 2" ~: parseJqQuery "[.hoge,.piyo]" ~?= Right (JqQueryArray [JqQueryFilter (JqField "hoge" JqNil), JqQueryFilter (JqField "piyo" JqNil)]), -- [{hoge: ""},{piyo: ""}]
      "jqQueryParser test 3" ~: parseJqQuery "{\"hoge\":[],\"piyo\":[]}" ~?= Right (JqQueryObject [("hoge", JqQueryArray []), ("piyo", JqQueryArray [])]) -- [{hoge: []},{piyo: []}]
    ]
...
jqQueryParserSpacesTest =
  TestList
    [ "jqQueryParser spaces test 1" ~: parseJqQuery " [ ] " ~?= Right (JqQueryArray []),
      "jqQueryParser spaces test 2" ~: parseJqQuery " [ . hoge , . piyo ] " ~?= Right (JqQueryArray [JqQueryFilter (JqField "hoge" JqNil), JqQueryFilter (JqField "piyo" JqNil)]),
      "jqQueryParser spaces test 3" ~: parseJqQuery "{ \"hoge\" : [ ] , \"piyo\" : [ ] } " ~?= Right (JqQueryObject [("hoge", JqQueryArray []), ("piyo", JqQueryArray [])])
    ]
...
-- applyFilter関数に文字列のクエリを与えた結果として、testDataを正しく解釈できるかテスト
...
-- unsafeParseFilterと同様、パースに失敗した場合errorを返すためunsafe
